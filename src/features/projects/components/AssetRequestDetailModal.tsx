...
const workflowStages: Array<{ value: AssetWorkflowStage; label: string }> = [
  { value: 'QC_LIBRARY', label: 'QC Library' },
  { value: 'GEN_REFINE', label: 'Gen/Refine' },
  { value: 'QC_EDIT', label: 'QC/Edit' },
  { value: 'CLIENT_REVIEW', label: 'Client Review' },
  { value: 'QC_DISTRO', label: 'QC/Distro' },
]
  ...
  if (isEditing) {
    return (
      <select
        ...
        disabled={isSaving}
        className="rounded-full px-2.5 py-0.5 text-xs font-medium border-gray-300 focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
      >
        {options.map((opt) => (
...
function WorkflowProgressBar({
  ...
  const currentIndex = workflowStages.findIndex(s => s.value === currentStage)

  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-gray-600 mb-1">
        <span>Workflow Progress</span>
        <span>{currentIndex + 1} of {workflowStages.length}</span>
      </div>
      ...
      <div className="flex justify-between text-xs">
        {workflowStages.map((stage, index) => (
          <button
            ...
            onClick={() => onStageChange(stage.value)}
            className={cn(
              "flex-1 text-center transition-colors hover:text-blue-600",
...
function DurationSelector({
  ...
  return (
    <div className="space-y-2">
      <EditableField
        label="Duration"
        value={showCustom ? 'CUSTOM' : (value ? String(value) : '')}
        type="select"
        options={selectOptions}
        onSave={async (val) => {
          if (val === 'CUSTOM') {
...
export function AssetRequestDetailModal({
  ...
  return (
    ...
    >
      {/* ── HEADER BAR ── */}
      <div className="flex items-start justify-between gap-3 pb-2.5 border-b border-gray-300 mb-3">
      ...
      <div className="max-h-[80vh] overflow-y-auto">
        <div className="space-y-3">
          ...
          {/* ── PROPERTIES STRIP ── */}
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 py-2.5 px-3 bg-gray-200/60 border border-gray-300 rounded-lg">
            <div className="flex items-center gap-1.5">
              <span className="text-xs text-gray-500">Client:</span>
              <Avatar name={assetRequest.clientAssignedEmail || 'U'} size="sm" className="!h-5 !w-5 !text-[9px]" />
              <EditableField
                label=""
                value={assetRequest.clientAssignedEmail || ''}
                type="email"
                onSave={(value) => handleUpdate('clientAssignedEmail', value)}
                placeholder="Client email"
              />
            </div>
            <span className="text-gray-400">·</span>
            <div className="flex items-center gap-1.5">
              <span className="text-xs text-gray-500">Kartel:</span>
              <Avatar name={assetRequest.kartelAssignedEmail || 'K'} size="sm" className="!h-5 !w-5 !text-[9px]" />
              <EditableField
                label=""
                value={assetRequest.kartelAssignedEmail || ''}
                type="email"
                onSave={(value) => handleUpdate('kartelAssignedEmail', value)}
                placeholder="Kartel email"
              />
            </div>
          </div>

          {/* ── PROGRESS BARS ── */}
          <div className="space-y-3 py-2.5 px-3 bg-gray-200/60 border border-gray-300 rounded-lg">
            <WorkflowProgressBar
          ...
          <div className="grid grid-cols-5 gap-4">
            ...
            <div className="col-span-3 space-y-3">
              ...
              {/* Asset Specs Card */}
              <div className="rounded-lg border border-gray-300 bg-gray-200/60 p-3 space-y-3">
                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Asset Specs</h4>
                ...
                />
                {assetRequest.creativeType?.toLowerCase().includes('video') && (
                  <DurationSelector
            ...
            <div className="col-span-2">
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Feedback & Notes</h4>
          ...
          <div className="border-t border-gray-300 pt-2.5">
            ...
            >
              <svg
                className={cn('h-3.5 w-3.5 transition-transform', showDetails && 'rotate-90')}
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            {showDetails && (
              <div className="mt-3 space-y-4">
                ...
                {false && (
                <div>
                  <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Linked Files</h4>
                  {loadingUploads ? (
                ...
                {/* Version History */}
                <div>
                  <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Version History</h4>
                  <div className="space-y-2">
                ...
                <div className="text-xs text-gray-500 flex flex-wrap gap-x-3 gap-y-1 pt-1 border-t border-gray-100">
